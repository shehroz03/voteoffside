// Personal life data for top players — publicly known facts
// Photos: Wikipedia Commons with referrerPolicy="no-referrer" to bypass hotlink check
export const personalLife = {

  'lionel-messi-arg': {
    partner: {
      name: 'Antonela Roccuzzo',
      role: 'Wife',
      photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/0/03/LIONEL_MESSI_-_BRESH_MIAMI.jpg/500px-LIONEL_MESSI_-_BRESH_MIAMI.jpg',
      since: '2017',
      note: 'Childhood sweethearts — known each other since age 5 in Rosario, Argentina.',
    },
    marriages: [
      { name: 'Antonela Roccuzzo', date: 'June 30, 2017', place: 'Rosario, Argentina', status: 'Married ✅' },
    ],
    children: [
      { name: 'Thiago Messi',  born: 'Nov 2, 2012', with: 'Antonela Roccuzzo', emoji: '👦' },
      { name: 'Mateo Messi',   born: 'Sep 11, 2015', with: 'Antonela Roccuzzo', emoji: '👦' },
      { name: 'Ciro Messi',    born: 'Mar 10, 2018', with: 'Antonela Roccuzzo', emoji: '👦' },
    ],
    relationshipHistory: [
      { name: 'Antonela Roccuzzo', years: '2008 – Present', status: 'Wife ✅' },
    ],
  },

  'cristiano-ronaldo-por': {
    partner: {
      name: 'Georgina Rodríguez',
      role: 'Long-term Partner',
      photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Georgina_Rodr%C3%ADguez_in_the_Oval_Office_%2854934212986%29_%28cropped%29.jpg/500px-Georgina_Rodr%C3%ADguez_in_the_Oval_Office_%2854934212986%29_%28cropped%29.jpg',
      since: '2016',
      note: 'Met at a Gucci store in Madrid in 2016. They have been together ever since.',
    },
    marriages: [
      { name: 'Never officially married', date: '—', place: '—', status: 'Unmarried (together since 2016)' },
    ],
    children: [
      { name: 'Cristiano Jr.', born: 'Jun 17, 2010', with: 'Surrogate', emoji: '👦' },
      { name: 'Eva Maria',      born: 'Jun 8, 2017',  with: 'Surrogate (twin)', emoji: '👧' },
      { name: 'Mateo',          born: 'Jun 8, 2017',  with: 'Surrogate (twin)', emoji: '👦' },
      { name: 'Alana Martina',  born: 'Nov 12, 2017', with: 'Georgina Rodríguez', emoji: '👧' },
      { name: 'Bella Esmeralda',born: 'Apr 18, 2022', with: 'Georgina Rodríguez', emoji: '👧' },
    ],
    relationshipHistory: [
      { name: 'Merche Romero',      years: '2004–2006', status: 'Ex-girlfriend' },
      { name: 'Nereida Gallardo',   years: '2008', status: 'Ex-girlfriend' },
      { name: 'Irina Shayk',        years: '2010–2015', status: 'Ex-girlfriend (model)', photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Irina_Shayk_Met_Gala_2022_%28cropped%29.jpg/320px-Irina_Shayk_Met_Gala_2022_%28cropped%29.jpg' },
      { name: 'Georgina Rodríguez', years: '2016 – Present', status: 'Current partner ✅' },
    ],
  },

  'neymar-bra': {
    partner: {
      name: 'Bruna Biancardi',
      role: 'Girlfriend',
      photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/8/8e/Bruna_Biancardi_2022.jpg/500px-Bruna_Biancardi_2022.jpg',
      since: '2022',
      note: 'Brazilian model and influencer. They reconciled after a brief split in 2023.',
    },
    marriages: [
      { name: 'Never married', date: '—', place: '—', status: 'Unmarried' },
    ],
    children: [
      { name: 'Davi Lúcio', born: 'Aug 24, 2011', with: 'Carolina Dantas', emoji: '👦' },
      { name: 'Mavie',       born: 'Oct 6, 2023',  with: 'Bruna Biancardi', emoji: '👧' },
    ],
    relationshipHistory: [
      { name: 'Carolina Dantas',   years: '2010–2011', status: 'Ex (mother of Davi)' },
      { name: 'Bruna Marquezine', years: '2012–2018 (on/off)', status: 'Ex-girlfriend (actress)', photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/37/Bruna_Marquezine_2019_%28cropped%29.jpg/320px-Bruna_Marquezine_2019_%28cropped%29.jpg' },
      { name: 'Natalia Barulich', years: '2019–2020', status: 'Ex-girlfriend (model)' },
      { name: 'Bruna Biancardi',  years: '2022 – Present', status: 'Current girlfriend ✅' },
    ],
  },

  'kylian-mbapp-fra': {
    partner: {
      name: 'Keeps private life private',
      role: '—',
      photo: null,
      since: '—',
      note: 'Mbappé is famously private about his romantic life and has not confirmed any public relationship.',
    },
    marriages: [
      { name: 'Never married', date: '—', place: '—', status: 'Single / Private' },
    ],
    children: [],
    relationshipHistory: [
      { name: 'Ines Rau', years: '2022 (rumoured)', status: 'Rumoured', photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/52/Ines_Rau.jpg/320px-Ines_Rau.jpg' },
      { name: 'Emma Smet', years: '2023 (rumoured)', status: 'Rumoured' },
    ],
  },

  'erling-haaland-nor': {
    partner: {
      name: 'Isabel Johansen',
      role: 'Girlfriend',
      photo: null,
      since: '2022',
      note: 'Norwegian girlfriend who prefers to stay completely out of the spotlight and public eye.',
    },
    marriages: [
      { name: 'Never married', date: '—', place: '—', status: 'Unmarried' },
    ],
    children: [],
    relationshipHistory: [
      { name: 'Isabel Johansen', years: '2022 – Present', status: 'Current girlfriend ✅' },
    ],
  },

  'vinicius-junior-bra': {
    partner: {
      name: 'Maria Júlia Mazalli',
      role: 'Girlfriend (on/off)',
      photo: null,
      since: '2022',
      note: 'Brazilian model and influencer. Their relationship has been on-and-off throughout 2022–2024.',
    },
    marriages: [
      { name: 'Never married', date: '—', place: '—', status: 'Unmarried' },
    ],
    children: [],
    relationshipHistory: [
      { name: 'Kainara Veras',      years: '2019–2021', status: 'Ex-girlfriend' },
      { name: 'Maria Júlia Mazalli', years: '2022 – Present', status: 'Current girlfriend ✅' },
    ],
  },

  'kevin-de-bruyne-bel': {
    partner: {
      name: 'Michèle Lacroix',
      role: 'Wife',
      photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/7/7b/Michele_Lacroix.jpg/320px-Michele_Lacroix.jpg',
      since: '2014',
      note: 'Belgian physiotherapist. Married in a lavish ceremony in Sorrento, Italy in 2017.',
    },
    marriages: [
      { name: 'Michèle Lacroix', date: 'June 2017', place: 'Sorrento, Italy', status: 'Married ✅' },
    ],
    children: [
      { name: 'Mason Milian', born: '2016', with: 'Michèle Lacroix', emoji: '👦' },
      { name: 'Rome',          born: '2018', with: 'Michèle Lacroix', emoji: '👦' },
      { name: 'Suri',          born: '2020', with: 'Michèle Lacroix', emoji: '👧' },
    ],
    relationshipHistory: [
      { name: 'Michèle Lacroix', years: '2014 – Present', status: 'Wife ✅' },
    ],
  },

  'robert-lewandowski-pol': {
    partner: {
      name: 'Anna Lewandowska',
      role: 'Wife',
      photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/98/Anna_Lewandowska_%28cropped%29.jpg/500px-Anna_Lewandowska_%28cropped%29.jpg',
      since: '2007',
      note: 'Polish karateka (3× world champion), personal trainer, nutritionist and entrepreneur.',
    },
    marriages: [
      { name: 'Anna Lewandowska', date: 'June 22, 2013', place: 'Serock, Poland', status: 'Married ✅' },
    ],
    children: [
      { name: 'Klara',  born: 'May 4, 2017',  with: 'Anna Lewandowska', emoji: '👧' },
      { name: 'Laura',  born: 'May 6, 2020',  with: 'Anna Lewandowska', emoji: '👧' },
    ],
    relationshipHistory: [
      { name: 'Anna Lewandowska', years: '2007 – Present', status: 'Wife ✅' },
    ],
  },

  'jude-bellingham-eng': {
    partner: {
      name: 'Laura Celia Valk',
      role: 'Girlfriend',
      photo: null,
      since: '2024',
      note: 'Dutch model and influencer. Jude is very private about his personal life.',
    },
    marriages: [
      { name: 'Never married', date: '—', place: '—', status: 'Unmarried' },
    ],
    children: [],
    relationshipHistory: [
      { name: 'Asantewa Chitty', years: '2022–2023', status: 'Ex-girlfriend' },
      { name: 'Laura Celia Valk', years: '2024 – Present', status: 'Current girlfriend ✅' },
    ],
  },

  'harry-kane-eng': {
    partner: {
      name: 'Katie Goodland',
      role: 'Wife',
      photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/a0/Katie_Goodland.jpg/320px-Katie_Goodland.jpg',
      since: '2011',
      note: 'Childhood sweethearts who went to the same school in Chingford, Essex.',
    },
    marriages: [
      { name: 'Katie Goodland', date: 'June 2019', place: 'Bahamas', status: 'Married ✅' },
    ],
    children: [
      { name: 'Ivy Jane',    born: 'Jan 8, 2017',  with: 'Katie Goodland', emoji: '👧' },
      { name: 'Vivienne',    born: 'Aug 2018',      with: 'Katie Goodland', emoji: '👧' },
      { name: 'Louis Harry', born: 'Dec 2020',      with: 'Katie Goodland', emoji: '👦' },
      { name: 'Henry',       born: 'Dec 2023',      with: 'Katie Goodland', emoji: '👦' },
    ],
    relationshipHistory: [
      { name: 'Katie Goodland', years: '2011 – Present', status: 'Wife ✅' },
    ],
  },
}
